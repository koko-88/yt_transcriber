...
import { parseJson3, parseSrv3, parseVtt, parseCaption } from '../src/core/parsers';
...
describe('parseJson3', () => {
  it('parses events with segment runs', () => {
    ...
    expect(segs![0]).toMatchObject({ startMs: 0, endMs: 1500, text: 'Hello world' });
...
describe('parseCaption', () => {
  it('dispatches on format', () => {
    expect(parseCaption('{"events":[{"tStartMs":0,"dDurationMs":100,"segs":[{"utf8":"hi"}]}]}')).not.toBeNull();
    expect(parseCaption('WEBVTT\n\n00:00.000 --> 00:01.000\nhi\n')).not.toBeNull();
    expect(parseCaption('<?xml version="1.0"?><timedtext><body><text start="0" dur="1">hi</text></body></timedtext>')).not.toBeNull();
  });
});
