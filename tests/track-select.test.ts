...
import { buildTrackEntries, selectTrack } from '../src/providers/youtube/track-select';
...
describe('buildTrackEntries', () => {
  ...
  it('preserves baseUrl for static fetch', () => {
    const entries = buildTrackEntries([track({ baseUrl: 'https://example.com/timedtext' })]);
...
describe('selectTrack', () => {
  ...
  it('matches by language prefix (en-US matches en)', () => {
    const enUS = buildTrackEntries([track({ languageCode: 'en-US', label: 'English (US)' })]);
