...
function assertTrustedContext() {
  ...
  if (typeof window !== 'undefined' && window.location && window.location.hostname === 'www.youtube.com') {
    throw new AppError({
      code: 'INTERNAL',
      message: 'Security Violation: Attempted to access secrets from untrusted content script context',
    });
  }
}
...
export async function getSecret(providerId: string): Promise<Secret | null> {
  ...
  try {
    ...
    const sessionData = await browser.storage.session.get(`secret_${providerId}`) as Record<string, unknown>;
    ...
    logger.error('storage', 'Failed to get secret', { providerId, error: err.message });
    return null;
  }
}
 ...
 */
export async function setSecret(providerId: string, key: string, sessionOnly: boolean = false): Promise<void> {
  assertTrustedContext();
  ...
  try {
    ...
    logger.error('storage', 'Failed to save secret', { providerId, error: err.message });
    throw new AppError({
...
export async function deleteSecret(providerId: string): Promise<void> {
  ...
  try {
    ...
    logger.error('storage', 'Failed to delete secret', { providerId, error: err.message });
  }
}
