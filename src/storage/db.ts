  ...
  if (!dbPromise) {
    dbPromise = openDB<WorkbenchDB>(DB_NAME, DB_VERSION, {
      upgrade(db, oldVersion, newVersion, transaction) {
        logger.info('storage', `Upgrading database from ${oldVersion} to ${newVersion}`);
        ...
        if (oldVersion < 1) {
          ...
          const videoTags = db.createObjectStore('video_tags', { keyPath: ['videoId', 'tagId'] });
      ...
      logger.error('storage', 'Failed to open database', { error: err.message });
      dbPromise = null;
      throw new AppError({
