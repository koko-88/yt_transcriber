...
export function TranscriptView() {
  ...
  const filtered: readonly TranscriptSegment[] = useMemo(() => {
    ...
    return searchSegments(transcript.segments, s.searchQuery).map((r) => r.segment);
  ...
  const itemCount = s.viewMode === 'paragraph' ? paragraphs.length : filtered.length;
  ...
  if (s.availability !== 'available' || !transcript) {
    ...
    return (
      <div className="view">
        <div className="banner" data-tone={s.availability === 'unknown' ? 'error' : undefined}>
          {s.loading ? (
            <span>
        ...
        {(s.availability === 'fetch-empty' || s.availability === 'needs-player-interaction') && (
  ...
  const doExport = (format: string) => {
    ...
    switch (format) {
      case 'txt':
        download(makeExportFilename(title, lang, 'txt'), exportTxt(transcript), 'text/plain');
        break;
      case 'md':
        download(makeExportFilename(title, lang, 'md'), exportMarkdown(transcript), 'text/markdown');
        break;
      case 'srt':
        download(makeExportFilename(title, lang, 'srt'), exportSrt(transcript.segments), 'application/x-subrip');
        break;
      case 'vtt':
        download(makeExportFilename(title, lang, 'vtt'), exportVtt(transcript.segments), 'text/vtt');
        break;
      case 'json':
        download(makeExportFilename(title, lang, 'json'), exportJson(transcript), 'application/json');
        break;
    }
  };
    ...
    <div className="view" style={{ padding: 0 }}>
      <div style={{ padding: '12px 12px 0', display: 'flex', flexDirection: 'column', gap: 10 }}>
        <div className="meta">
          ...
          {transcript.video.channelName && <div className="channel">{transcript.video.channelName}</div>}
        </div>
          ...
          >
            {s.tracks.map((tr) => (
              <option key={tr.trackId} value={tr.trackId}>
                {tr.languageLabel} ({tr.kind === 'manual' ? s.tr('transcript.tracks.manual') : tr.kind === 'asr' ? s.tr('transcript.tracks.asr') : s.tr('transcript.tracks.translated')})
              </option>
            ))}
          </select>
        </div>
        ...
        <div className="toolbar">
          <button
            ...
            onClick={() => s.setViewMode(s.viewMode === 'paragraph' ? 'raw' : 'paragraph')}
          >
            {s.viewMode === 'paragraph' ? s.tr('transcript.view.paragraph') : s.tr('transcript.view.raw')}
          </button>
          <button className="btn" aria-pressed={s.follow} onClick={() => s.setFollow(!s.follow)}>
            {s.tr('transcript.follow')}
          </button>
          <button className="btn" onClick={() => void navigator.clipboard.writeText(copyPlainText(transcript.segments))}>
            {s.tr('transcript.copy.text')}
          </button>
          <button className="btn" onClick={() => void navigator.clipboard.writeText(copyWithTimestamps(transcript.segments))}>
          ...
          {s.videoId && !s.savedVideoIds.has(s.videoId) && (
            <button className="btn" onClick={() => void s.saveCurrentToLibrary()}>
              {s.tr('library.save')}
            </button>
          )}
        </div>
        ...
        <div style={{ height: virtualizer.getTotalSize(), position: 'relative' }}>
          {virtualizer.getVirtualItems().map((vi) => {
            ...
            if (s.viewMode === 'paragraph') {
              ...
              if (!p) return null;
              return (
                <p key={vi.key} className="paragraph" style={style} ref={virtualizer.measureElement} data-index={vi.index}>
            ...
            if (!seg) return null;
            const active = s.playbackMs >= seg.startMs && s.playbackMs < seg.endMs;
            return (
              <button
                key={vi.key}
