...
interface Candidate { startMs: number; endMs: number; text: string }

function midpoint(cue: Candidate): number { return (cue.startMs + cue.endMs) / 2; }
function words(text: string): string[] { return text.trim().split(/\s+/).filter(Boolean); }
function key(word: string): string { return word.toLocaleLowerCase().replace(/[^\p{L}\p{N}]/gu, ""); }
...
function duplicatePrefix(previous: string, current: string): number {
  ...
  for (let count = Math.min(left.length, right.length); count > 0; count--) {
    if (left.slice(-count).every((word, index) => key(word) === key(right[index]!))) return count;
  }
  return 0;
}
...
export class TranscriptNormalizer {
  ...
  addWindow(window: PcmWindow, cues: SttCue[]): void {
    const candidates = cues.flatMap((cue): Candidate[] => {
      ...
      const to = Number(cue.timestamp[1] ?? window.endSeconds - window.startSeconds);
      if (!text || !Number.isFinite(from) || !Number.isFinite(to)) return [];
      const startMs = Math.round(Math.max(window.startSeconds, window.startSeconds + from) * 1000);
      const endMs = Math.round(Math.min(window.endSeconds, window.startSeconds + to) * 1000);
      return endMs > startMs ? [{ startMs, endMs, text }] : [];
    });
    if (this.pending !== null) {
      const splitMs = Math.round((window.startSeconds + OVERLAP_SECONDS / 2) * 1000);
      for (const cue of this.pending.filter((candidate) => midpoint(candidate) < splitMs))
        this.append(cue);
      this.pending = candidates.filter((candidate) => midpoint(candidate) >= splitMs);
    } else {
