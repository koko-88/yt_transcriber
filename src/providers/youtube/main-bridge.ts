...
interface YtPlayerLike {
  ...
  getVideoData?: () => { video_id?: string; title?: string; author?: string; channelId?: string };
...
function getPlayer(): YtPlayerLike | null {
  const el = document.getElementById('movie_player') as unknown as YtPlayerLike | null;
  return el ?? null;
}

function getPlayerResponse(player: YtPlayerLike | null): Record<string, unknown> | null {
  try {
    const viaApi = player?.getPlayerResponse?.() as Record<string, unknown> | undefined;
    if (viaApi && typeof viaApi === 'object') return viaApi;
  } catch {
    /* fall through */
  }
  const w = window as unknown as { ytInitialPlayerResponse?: Record<string, unknown> };
...
export function snapshotFromPlayerResponse(
  ...
  const playability = (pr?.['playabilityStatus'] ?? {}) as Record<string, unknown>;
  const captions = pr?.['captions'] as Record<string, unknown> | undefined;
  const renderer = captions?.['playerCaptionsTracklistRenderer'] as Record<string, unknown> | undefined;
  const rawTracks = (renderer?.['captionTracks'] as RawCaptionTrack[] | undefined) ?? [];
  ...
  const tracks: BridgeTrack[] = rawTracks
    ...
    .map((t) => ({
      ...
      kind: t.kind,
      vssId: t.vssId,
      baseUrl: typeof t.baseUrl === 'string' ? t.baseUrl.slice(0, 4096) : undefined,
      isTranslatable: t.isTranslatable,
    }));

  let videoId: string | null = typeof vd['videoId'] === 'string' ? (vd['videoId'] as string) : null;
  let title: string | null = typeof vd['title'] === 'string' ? (vd['title'] as string) : null;
  let channelName: string | null = typeof vd['author'] === 'string' ? (vd['author'] as string) : null;
  let channelId: string | null = typeof vd['channelId'] === 'string' ? (vd['channelId'] as string) : null;
  try {
  ...
  const durationSeconds = typeof lenRaw === 'string' || typeof lenRaw === 'number' ? Number(lenRaw) : null;
  ...
  return {
    ...
    playabilityStatus: typeof playability['status'] === 'string' ? (playability['status'] as string) : null,
    playabilityReason: typeof playability['reason'] === 'string' ? (playability['reason'] as string) : null,
    tracks,
    captionsApiAvailable: !!player && typeof player.setOption === 'function' && typeof player.getOption === 'function',
  };
}
