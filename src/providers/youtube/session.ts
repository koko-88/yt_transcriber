...
export function videoIdFromUrl(url: string): string | null {
  try {
    const u = new URL(url);
    if (u.hostname !== 'www.youtube.com' && u.hostname !== 'm.youtube.com') return null;
    if (u.pathname === '/watch') {
...
export function metadataFromSnapshot(snapshot: PlayerSnapshot): VideoMetadata | null {
...
async function fetchCaption(url: string, signal: AbortSignal): Promise<{ status: number; body: string }> {
...
export function startYouTubeSession(): void {
  ...
  let inFlight: { key: string; controller: AbortController; promise: Promise<AcquisitionResult> } | null = null;
  ...
  bus.on('acq.getState', z.object({}), ['extension-page'], async (): Promise<VideoPageState> => {
    const videoId = videoIdFromUrl(location.href);
    if (!videoId) return { videoId: null, availability: 'not-a-video-page', tracks: [], metadata: null };
    const snapshot = await currentSnapshot();
    if (!snapshot) return { videoId, availability: 'unsupported-page-structure', tracks: [], metadata: null };
    return {
      videoId,
  ...
  bus.on('acq.acquire', z.object({ trackId: z.string().optional() }), ['extension-page'], async (payload): Promise<AcquisitionResult> => {
    const videoId = videoIdFromUrl(location.href);
    if (!videoId) {
      return { ok: false, reason: 'not-a-video-page', retryable: false, diagnostics: [] };
    }
  ...
  bus.on('acq.seek', z.object({ timeMs: z.number().int().nonnegative() }), ['extension-page'], async (payload) => {
