...
export function safeMediaUrl(raw: string): string | null {
  try {
    ...
    if (url.protocol !== "https:" || !/(^|\.)googlevideo\.com$/.test(url.hostname) ||
        url.pathname !== "/videoplayback") return null;
    const expiry = Number(url.searchParams.get("expire"));
    if (Number.isFinite(expiry) && expiry > 0 && expiry * 1000 <= Date.now()) return null;
...
const directProvider: MediaSourceProvider = {
  ...
  resolve({ videoId, response, metadata }) {
    ...
    for (const format of formats) {
      ...
      if (url) return { videoId, url, mimeType: format.mimeType, metadata, strategy: this.id };
    }
    return null;
  },
};
...
const observedProvider: MediaSourceProvider = {
  ...
  resolve({ videoId, response, observed, metadata }) {
    ...
    const formats = (response["streamingData"] as { adaptiveFormats?: unknown[] } | undefined)?.adaptiveFormats ?? [];
    ...
    const matched = formats.find((entry) => {
      const format = entry as { itag?: number; mimeType?: string; url?: string;
        signatureCipher?: string; cipher?: string };
      if (!/^(audio|video)\/(webm|mp4)/.test(format.mimeType ?? "")) return false;
      if (format.itag == null || String(format.itag) !== observedUrl.searchParams.get("itag")) return false;
      const cipher = format.signatureCipher ?? format.cipher;
      const playerUrl = format.url ?? (cipher ? new URLSearchParams(cipher).get("url") : null);
      if (!playerUrl) return false;
      try {
        const expected = new URL(playerUrl);
        return expected.hostname === observedUrl.hostname &&
    ...
    return matched ? { videoId, url, mimeType: (matched as { mimeType: string }).mimeType,
      metadata, strategy: this.id } : null;
  },
};

const providers: readonly MediaSourceProvider[] = [directProvider, observedProvider];
