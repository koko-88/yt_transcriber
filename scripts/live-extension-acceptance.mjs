    ...
    try {
      ...
      const before = await video.evaluate(() => {
        ...
        return {
          ...
          captionsPressed: document.querySelector(".ytp-subtitles-button")?.getAttribute("aria-pressed"),
      ...
      const copy = panel.getByRole("button", { name: "Copy text" });
      const noCaptions = panel.getByText("No YouTube captions are available", { exact: false });
      await Promise.race([
        copy.waitFor({ timeout: 45_000 }),
      ...
      if (await noCaptions.isVisible()) {
        console.log(JSON.stringify({ videoId, state: "NO_CAPTIONS", elapsedMs, before, after }));
        continue;
      }
      const downloadPromise = panel.waitForEvent("download");
      await panel.getByLabel("Export").selectOption("json");
      const download = await downloadPromise;
      const transcript = JSON.parse(readFileSync(await download.path(), "utf8"));
      ...
      const raw = await panel?.evaluate(async () => {
        const state = await chrome.runtime.sendMessage({ type: "acq.getState", payload: {} });
        const acquisition = await chrome.runtime.sendMessage({ type: "acq.acquire", payload: {} });
        return {
          state: state.ok ? state.data : state.error,
          acquisition: acquisition.ok
            ? acquisition.data.ok
              ? { ok: true, segmentCount: acquisition.data.transcript.segments.length }
              : acquisition.data
            : acquisition.error,
        };
      ...
      console.log(JSON.stringify({
        videoId,
        error: String(error),
        banner: await panel?.locator(".banner").allTextContents().catch(() => []),
        panelText: (await panel?.locator("body").innerText().catch(() => ""))?.slice(0, 800),
        player: await video.evaluate(() => {
          const p = document.querySelector("#movie_player");
          return { state: p?.getPlayerState?.(), track: p?.getOption?.("captions", "track") };
        }).catch(() => null),
        raw,
      }));
    } finally {
